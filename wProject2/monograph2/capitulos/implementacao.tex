...
\begin{algorithm}[H]\label{alg:getBiomeXZ}
    ...
    $biomaTipo = $trunc$(areaValue \cdot \bar{N})$\;\tcc{$\bar{N} = 5$ Quantidade de biomas distintos}
...
$\{(di+1, dj+1), (di+1, dj), (di+1, dj-1), (di, dj+1), (di, dj-1), (di-1, dj+1), (di-1, dj), (di-1, dj-1)\}$
, é possivel fazer com que cada bioma faça suavisação com apenas 3 vizinhos, no caso do 
$(di, dj)$, fará com $\{(di+1, dj), (di, dj+1), (di+1, dj+1)\}$, assim cada 
...
\begin{algorithm}[H]\label{alg:borderFinal}%Usando parâmetro e não k
    ...
    \If {$BH_{x}.bordaFlag \land BH_{x}.acimaFlag$}{
        $Bioma_{xp}$ = Algoritmo\ref{alg:getBiomeXZ}($x + l, z$)\;
        $afxy = k/f_{Bioma_{xp}}$\;
        $h'_{xp} =$ perlinNoise$(x/afxy, z/afxy, \theta_{Bioma_{xp}})$\;
        $h_{xp} =$ Algoritmo\ref{alg:hBioValuation}($Bioma_{xp}$, $h'_{xp}$)\;
        $cor_{xp} = $Algoritmo\ref{alg:colorValuation}$(minColor_{Bioma_{xp}}, midColor_{Bioma_{xp}}, maxColor_{Bioma_{xp}})$\;
        \tcc{Interpolação linear}
        $h = $ mix($h_{xp}, h_{p}, BH_{x}.pDist$)\;
        $cor $=  mix($cor_{xp}, cor_{p}, BH_{x}.pDist$)\;
    }
    \tcc{Caso2: $(x, z)$ é fronteira em $z$}
    \If {$BH_{z}.bordaFlag \land BH_{z}.acimaFlag$}{
        $Bioma_{zp}$ = Algoritmo\ref{alg:getBiomeXZ}($x, z + l$)\;
        $afxy = k/f_{Bioma_{zp}}$\;
        $h'_{zp} =$ perlinNoise$(x/afxy, z/afxy, \theta_{Bioma_{zp}})$\;
        $h_{zp} =$ Algoritmo\ref{alg:hBioValuation}($Bioma_{zp}$, $h'_{zp}$)\;
        $cor_{zp} = $Algoritmo\ref{alg:colorValuation}$(minColor_{Bioma_{zp}}, midColor_{Bioma_{zp}}, maxColor_{Bioma_{zp}})$\;
        \tcc{Interpolação linear}
        $h = $ mix($h_{zp}, h_{p}, BH_{z}.pDist$)\;
        $cor $=  mix($cor_{zp}, cor_{p}, BH_{z}.pDist$)\;
    }
    \tcc{Caso 3: $(x, z)$ é fronteira em $x$ e $z$}
    \If {$BH_{z}.bordaFlag \land BH_{x}.bordaFlag \land BH_{z}.acimaFlag \land BH_{x}.acimaFlag$}{
        $Bioma_{xzp}$ = Algoritmo\ref{alg:getBiomeXZ}($x + l, z + l$)\;
        $afxy = k/f_{Bioma_{xzp}}$\;
        $h'_{xzp} =$ perlinNoise$(x/afxy, z/afxy, \theta_{Bioma_{xzp}})$\;
        $h_{xzp} =$ Algoritmo\ref{alg:hBioValuation}($Bioma_{xzp}$, $h'_{xzp}$)\;
        $cor_{xzp} = $Algoritmo\ref{alg:colorValuation}$(minColor_{Bioma_{xzp}}, midColor_{Bioma_{xzp}}, maxColor_{Bioma_{xzp}})$\;
        \tcc{Interpolação biliniar com os 4 biomas}
        $h_{i1} = $mix($h_{xzp}, h_{xp}, BH_{z}.pDist$)\;
        $cor_{i1} = $mix($cor_{xzp}, cor_{xp}, BH_{z}.pDist$)\;
        $h_{i2} = $ mix($h_{zp}, h_{p}, BH_{z}.pDist$)\;
        $cor_{i2} =$  mix($cor_{zp}, cor_{p}, BH_{z}.pDist$)\;
        $h =$ mix($h_{i1}, h_{i2}, BH_{x}.pDist$)\;
        $cor =$ mix($cor_{i1}, cor_{i2}, BH_{x}.pDist$)\;
    }
    
    \caption{Suavizando fronteiras.}
\end{algorithm}
