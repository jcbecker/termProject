...
\subsection{Cores}
Como em um jogo o terreno é representado por segmentos de planos, polignos preenchidos e não um conjunto
...
\textit{Perlin Noise}, já nas equeções \ref{eq:thePerlinNoisedesg1} e \ref{eq:thePerlinNoisedesg2}
...
\begin{equation}\label{eq:thePerlinNoise}
  ...
  \end{split}
\end{equation}
A equação \ref{eq:thePerlinNoisedesg1} apresenta um caso desgenerado caso a função
\textit{Noise} sempre retornar $1$, o resultado de $2^{-\theta} \cdot (2^{\theta +1}-1)$
apresenta o maior valor possivel retornado na função \textit{perlinNoise} em função da quantidade 
