...
O Algoritmo \ref{alg:genVectors}:(Construção da coleção de vértices e índices) monta a estrutura $V$ com $k^2$ vértices, para cada $v \in V$
é determinado $(x, z)$ e usa como $y$ o resultado do Algoritmo \ref{alg:vertexValuationm}:(Valoração de vértices). 
Para definir a cor de cada vértice também é usado Algoritmo \ref{alg:vertexValuationm}:(Valoração de vértices). Em seguida é montado o
\textit{array} $E$.
...
Considerando que Algoritmo \ref{alg:vertexValuationm}:(Valoração de vértices)($x, z$) retorne sempre a cor branca
e a altura $0.0$, por hora, os parâmetros $seed, b, l$ não terão influência, pois os mesmos 
só serão usados na função Algoritmo\ref{alg:vertexValuationm}:(Valoração de vértices). Portanto, fixando os parâmetros: $x_{s}, z_{s} = 0$, 
...
O Algoritmo \ref{alg:vertexValuationm}:(Valoração de vértices) será chamado para cada vértice $v \in V$. 
Este algoritmo será responsável para decidir os valores de $y$ e $cor$ associados
a cada $v$ no Algoritmo \ref{alg:genVectors}:(Construção da coleção de vértices e índices). A altura do vértice está representada
como $h$ no Algoritmo \ref{alg:vertexValuationm}:(Valoração de vértices).
...
\end{algorithm}
O Algoritmo \ref{alg:vertexValuationm}:(Valoração de vértices), recebe como parâmetro um
...
O Algoritmo \ref{alg:borderFinal}:(Interpolação nas fronteiras) é uma sequencia direta da função de valoração 
descrita acima(Algoritmo\ref{alg:vertexValuationm}:(Valoração de vértices)), ambos pertencem ao mesmo escopo, o Algoritmo \ref{alg:borderFinal}:(Interpolação nas fronteiras)
...
Supondo que o Algoritmo \ref{alg:vertexValuationm}:(Valoração de vértices) retorne sempre $h = (sin(x/10) + sin(z/10)) \cdot 4$ e 
a cor branca, o resultado seria a figura\ref{fig:sssins}. Assim podemos perceber o papel do Algoritmo \ref{alg:vertexValuationm}:(Valoração de vértices).
...
Supondo que o Algoritmo \ref{alg:vertexValuationm}:(Valoração de vértices) retorne sempre $h$ descrito na equação \ref{eq:thePerlinNoise223}
e cor branca, usando $afxz = k/f$, e usando os parâmetros descritos nas imagens \ref{fig:parametrosOctaves},
\ref{fig:parametrosfreq} e \ref{fig:parametrosSeed} podemos visualizar seus resultados nas próprias imagens. Nas imagens \ref{fig:parametrosOctaves}
é visto que quanto maior a quantidade de oitavas ($\theta$) mais enrugado fica o relevo. As imagens \ref{fig:parametrosfreq} mostram o comportamento do terreno com diferentes
frequências ($f$), nota-se que quanto maior o $f$ maior a perca e ganha de altitude em um determinado percurso. Nas imagens \ref{fig:parametrosSeed} é visível
o resultado com diferentes sementes ($seed$) para gerar o ruído.
...
Assim podemos
usar o Algoritmo \ref{alg:colorValuation}:(Coloração de vértices) para definir a cor do vértice baseado na altura.
$cor$ no algoritmo \ref{alg:colorValuation}:(Coloração de vértices) é o valor recebido pelo Algoritmo \ref{alg:genVectors}:(Construção da coleção de vértices e índices)
quando chama Algoritmo \ref{alg:vertexValuationm}:(Valoração de vértices)$(x_{s} + i, z_{s} + j).cor$.
...
Algoritmo \ref{alg:colorValuation}:(Coloração de vértices) porque estão em formatos diferentes, dessa forma temos
...
também tem uma função de manipulação do ruído, o Algoritmo \ref{alg:hBioValuation}:(Altura por bioma), 
...
Algoritmo \ref{alg:hBioValuation}:(Altura por bioma) é gerado mapas com um bioma,
...
o Algoritmo \ref{alg:getBiomeXZ}:(Escolhendo biomas em áreas de tamanho $b$).
...
o Algoritmo \ref{alg:borderTest}:(Teste de fronteira). O algoritmo recebe uma entrada $in$ e calcula
...
O Algoritmo \ref{alg:borderTest}:(Teste de fronteira) retorna uma estrutura $BH$, com as informações
...
No Algoritmo \ref{alg:borderFinal}:(Interpolação nas fronteiras), teremos casos em que:
\begin{itemize}
...
\begin{figure}[H]
    ...
    \caption{Algumas variáveis do algoritmo \ref{alg:borderFinal}:(Interpolação nas fronteiras)}
...
No Algoritmo \ref{alg:borderFinal}:(Interpolação nas fronteiras) as variáveis calculadas:
\begin{itemize}
