...
$fxz = k/f$\\
$Baseh = PerlinNoise(x/fxz, z/fxz, Octaves) * amp$\\
$Altura = Baseh * amp$
...
vizualização do mesmo. O valor $Baseh$ vai ficar com valores entre $-1$ e $1$, 
quando passar desse intervalo temos certeza que $-2 < Baseh > 2$. Então podemos
usar o seguinte algoritmo para definir a cor do vértice baseado na altura.

\begin{algorithm}[H]\label{alg:colorValuation}
    $auxNoise = clamp(Baseh, -1, 1)$\;
    \If {$auxNoise < 0$}{
        return $mix(midColor, minColor, auxNoise *-1)$\;
    }\Else{
